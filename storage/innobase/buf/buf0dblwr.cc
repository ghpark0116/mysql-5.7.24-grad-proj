...
start_again:
    ...
    ulint min_doublewrite_size =
        ((2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2 + 100) * UNIV_PAGE_SIZE);
    ...
    for (i = 0; i < 2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2; i++) {
        ...
        if (i == FSP_EXTENT_SIZE / 2) {
            ...
            mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_BLOCK1,
                             page_no, MLOG_4BYTES, &mtr);
            mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_REPEAT + TRX_SYS_DOUBLEWRITE_BLOCK1,
                             page_no, MLOG_4BYTES, &mtr);

        } else if (i == FSP_EXTENT_SIZE / 2 + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE) {
            ut_a(page_no == 2 * FSP_EXTENT_SIZE);
            mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_BLOCK2,
                             page_no, MLOG_4BYTES, &mtr);
            mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_REPEAT + TRX_SYS_DOUBLEWRITE_BLOCK2,
                             page_no, MLOG_4BYTES, &mtr);
    ...
    mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC,
                     ...
                     MLOG_4BYTES, &mtr);
    mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC + TRX_SYS_DOUBLEWRITE_REPEAT,
                     TRX_SYS_DOUBLEWRITE_MAGIC_N,
                     MLOG_4BYTES, &mtr);
...
void buf_dblwr_process(void) {
    ...
    for (recv_dblwr_t::list::iterator i = recv_dblwr.pages.begin();
        ...
        if (page_no >= space->size) {
            ...
            bool skip_warning =
                srv_is_tablespace_truncated(space_id) || srv_was_tablespace_truncated(space);
            ...
            } else if (buf_page_is_zeroes(read_buf, page_size) && !buf_page_is_zeroes(page, page_size) && !buf_page_is_corrupted(true, page, page_size, fsp_is_checksum_disabled(space_id))) {
    ...
    const page_t* page) /*!< in: page to check */
{
    if (memcmp(page + (FIL_PAGE_LSN + 4),
               page + (UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4),
               4)) {
